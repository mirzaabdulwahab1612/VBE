...
class VBEACB():
    ...
    def __init__(self,
        ...
        self.auxWeights = torch.nn.Sequential(torch.nn.Linear(448, self.num_rvecs, bias=False)).to(self.device)
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            next_states_features = self.wvec.representation(next_states_t).detach()
            response = self.auxWeights(next_states_features).detach() ** 2
            ...
            rand_labels = torch.Tensor(self.np_random.randn(len(states_t), self.num_rvecs)).to(self.device)
            current_states_features = self.wvec.representation(states_t).detach()
            rand_predictions = self.auxWeights.forward(current_states_features)
    ...
    def save_policy(self, path):
        ...
        torch.save(self.auxWeights.state_dict(), str(path+"auxWeights_at_step_"+str(self.time_step)))

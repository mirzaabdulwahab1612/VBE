...
def make_ensemble(num_actions: int,
  ...
  for _ in range(num_ensemble):
    network = snt.Sequential([
        snt.Linear(num_hidden_units, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(input_dims, 1/2)), seed=1+rnd_seed+_)),
        tf.nn.relu,
        snt.Linear(num_hidden_units, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(num_hidden_units, 1/2)), seed=2+rnd_seed+_)),
        tf.nn.relu,
        snt.Linear(num_actions, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(num_hidden_units, 1/2)), seed=3+rnd_seed+_))
    ])
    prior_network = snt.Sequential([
        snt.Linear(num_hidden_units, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(input_dims, 1/2)), seed=4+rnd_seed+_)),
        tf.nn.relu,
        snt.Linear(num_hidden_units, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(num_hidden_units, 1/2)), seed=5+rnd_seed+_)),
        tf.nn.relu,
        snt.Linear(num_actions, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(num_hidden_units, 1/2)), seed=6+rnd_seed+_))
    ])

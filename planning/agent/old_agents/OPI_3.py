...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.rand_wvec = self.np_random.multivariate_normal(np.zeros(self.feature_dim), np.eye(self.feature_dim), size=self.num_rvecs)
        self.rand_wvec = np.hstack((self.rand_wvec, self.rand_wvec))
        ...
        # error predictor
        self.evec = np.ones((self.num_rvecs, self.mem_size))
    ...
    def start(self, observation):
        ...
        next_act = self.optimistic_action(self.features, self.wvec, self.uvec, self.evec, target_action=False)
    ...
    def step(self, observation, reward, terminal):
        ...
        next_act = self.optimistic_action(self.features, self.wvec, self.uvec, self.evec, target_action=False, print_flag=True)
        if next_act is None:
            return next_act
        ...
        if(self.replay_buffer_train.get_buffer_size() > self.mini_batch_size and self.replay_buffer_test.get_buffer_size() > self.mini_batch_size):
            ...
            self.learn_error_estimates(num_updates=self.num_updates, mini_batch_size=self.mini_batch_size)
    ...
    def learn_error_estimates(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            current_state_target = np.dot(states, self.rand_wvec.T)
            current_state_pred = np.dot(states, self.rvec.T)
            current_state_error = np.abs(current_state_pred - current_state_target)

            next_values = get_values(next_states.T, self.evec, self.num_action, multi=True)
            next_q = np.sum(next_values, axis=-1) * (1.0/self.num_action)
            # next_q = np.sum(next_values, axis=-1)
            current_q = np.dot(states, self.evec.T)
            td_error_w_error = (current_state_error + self.gamma*terminals_*next_q) - current_q
            
            batch_errors_w_error = td_error_w_error.T.dot(states)
            self.evec = self.evec + ((self.alpha/mini_batch_size)* batch_errors_w_error)
    ...
    def learn_out_uncertainty_estimates(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            next_actions = self.optimistic_action(next_states, self.policy_wvec, self.policy_uvec, self.policy_evec, target_action=True)
            terminals_ = np.invert(terminals)
    ...
    def optimistic_action(self, state, wvec=None, uvec=None, evec=None, target_action=False, print_flag=False):
        if self.start_training:
            val = get_values(state, wvec, self.num_action)
            # unc = get_values(state, uvec, self.num_action)

            temp_iunc = []
            iunc = get_values(state.T, evec, self.num_action, multi=True)

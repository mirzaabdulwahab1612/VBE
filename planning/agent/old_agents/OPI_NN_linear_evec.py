...
class SARSA():
    ...
    def __init__(self, params):
        ...
        for i in range(self.num_rvecs):
            ...
            self.evec.append(LinearNet(self.feature_dim, self.num_action, zero_init=False))
            self.target_evec.append(LinearNet(self.feature_dim, self.num_action, zero_init=False))

        # Target networks
        self.target_wvec.load_state_dict(self.wvec.state_dict())
        for i in range(self.num_rvecs):
            self.target_rand_pred[i].load_state_dict(self.rand_pred[i].state_dict())
            self.target_evec[i].load_state_dict(self.evec[i].state_dict())
    ...
    def step(self, observation, reward, terminal):
        ...
        if(self.replay_buffer_train.get_buffer_size() > self.mini_batch_size and self.replay_buffer_test.get_buffer_size() > self.mini_batch_size):
            ...
            self.learn_evec(num_updates=self.num_updates, mini_batch_size=self.mini_batch_size)
    ...
    def learn_evec(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            states, actions, rewards, next_states, terminals = self.replay_buffer_test.sample_batch_lastdata(mini_batch_size)
            next_actions = self.optimistic_action(next_states, self.target_wvec, self.target_rand_pred, target_action=True)
            ...
            current_values = torch.stack([self.evec[id].forward(states_t) for id in range(self.num_rvecs)])
            current_q = current_values[:, np.arange(mini_batch_size), actions]
            next_values = torch.stack([self.evec[id].forward(next_states_t).detach() for id in range(self.num_rvecs)])
            next_q = next_values[:, np.arange(mini_batch_size), next_actions]

            target_q = iunc + self.gamma*terminals_*next_q
            loss = self.loss_fn(current_q, target_q)
...
def get_params():
    return ["optimizer_type", "num_rvecs", "alpha", "prior_mean", "p"]

class LinearNet(torch.nn.Module):
    def __init__(self, inputSize, outputSize, prior_mean=0.0, zero_init=False):
        ...
        if(zero_init):
            ...
            torch.nn.init.trunc_normal_(self.net.weight, mean=prior_mean, std=(1.0/np.power(inputSize, 1/2)))

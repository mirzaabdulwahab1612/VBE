...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.rvfs_target_polic_type = params.agent_params.rvfs_target_polic_type
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            # next_actions = torch.argmax(self.target_wvec.forward(next_states_t).detach(), dim=1)
            ...
            if self.rvfs_target_polic_type == "original":
                # Regular target policy for RVFs
                # next_actions = [next_actions] * self.num_rvecs
                rand_p = torch.stack([self.target_rand_pred[id].forward(next_states_t).detach() for id in range(self.num_rvecs)])
                ...
                next_actions = torch.argmax(iunc, dim=-1)
                next_actions = [next_actions] * self.num_rvecs
            elif self.rvfs_target_polic_type == "uniform":
                # Uniform target policy for RVFs
                next_actions = self.np_random.randint(0, self.num_action, (self.num_rvecs, mini_batch_size))
            else:
                exit("Invalid RVF target policy type")
            ...
            for j in rvfs_to_update:
                ...
                # No gamma for getting the random reward
                reward_u = current_q_target - (terminals_*next_q_target)
            ...
            if self.start_training:
                ...
                if not target_action:
                    if(np.argmax(state) == 0):
                        print(f"observation: {np.argmax(state)} val: {val} iunc: {iunc} bonus: {uncertainty_bonus} value_new: {value_new}")
            else:
...
def get_params():
    return ["optimizer_type", "rvfs_target_polic_type", "rvfs_update_all", "action_type", "num_rvecs", "alpha", "prior_mean", "p"]
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            if self.rvfs_target_polic_type == "original":
                ...
                # Uniform target policy for RVFs
                # next_actions = self.np_random.randint(0, self.num_action, (self.num_rvecs, mini_batch_size))
                next_actions = self.np_random.randint(0, self.num_action, mini_batch_size)
            else:
                exit("Invalid RVF target policy type")

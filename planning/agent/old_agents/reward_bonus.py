...
class SARSA():
    ...
    def step(self, observation, reward, terminal):
        ...
        # Getting state features of next S`
        get_features(observation, self.feature_constructor, self.features, self.features)
        # print(f"observation: {observation} , self.features: {self.features}")
    ...
    def learn_one_update(self, wvec=None, uvec=None, evec=None, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            temp_iunc = []
            iunc = np.dot(states, self.evec.T)
            # iunc = np.max(iunc, axis=1)
            id = np.argmax(iunc, axis=1)
            for k in range(self.num_action):
                temp_iunc.append(iunc[np.arange(self.mini_batch_size)][:, k])
            iunc = np.stack(temp_iunc, axis=1)
            iunc = iunc[np.arange(self.mini_batch_size), actions]



            td_error = (rewards + self.c * iunc) + self.gamma*terminals_*next_q - current_q
    ...
    def optimistic_action(self, state, wvec=None, uvec=None, evec=None, greedy=False, target_action=False, print_flag=False):
        ...
        if self.start_training:
            ...
            if(greedy):
                ...
                # iunc = get_values(state, evec, self.num_action)
                ...
                # unc = (np.exp(unc) / np.sum(np.exp(unc)))
                # iunc = (np.exp(iunc) / np.sum(np.exp(iunc)))
        ...
        if print_flag and self.print_mode and self.time_step%self.print_frequency==0:
            for i in range(self.num_action):
                ...
                # self.plotter.update_plot(copy.deepcopy(self.wvec[self.feature_dim:self.feature_dim+self.feature_dim]))
                # self.plotter.update_plot(copy.deepcopy(self.uvec[self.feature_dim:self.feature_dim+self.feature_dim]))

...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.bonus_type = params.agent_params.bonus_type
        self.action_type = params.agent_params.action_type
        if self.bonus_type == "target":
            self.c = 1.0
            self.p = params.agent_params.p
        else:
            self.c = params.agent_params.p
    ...
    def visualize(self):
        with torch.no_grad():
            ...
            for keys, data in vis_features.items():
                ...
                rand_t = np.array([self.p * self.rand_target[id].forward(observation).cpu().numpy() for id in range(self.num_rvecs)])
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            for j in range(self.num_rvecs):
                next_values_target = self.p * self.rand_target[j].forward(next_states_t).detach()
                next_q_target = next_values_target[np.arange(mini_batch_size), next_actions]
                current_values_target = self.p * self.rand_target[j].forward(states_t).detach()
    ...
    def optimistic_action(self, state, wvec=None, rand_pred=None, target_action=False, print_flag=False):
        with torch.no_grad():
            ...
            if self.start_training:
                ...
                rand_t = torch.stack([self.p * self.rand_target[id].forward(hidden_state) for id in range(self.num_rvecs)])
...
def get_params():
    return ["optimizer_type", "action_type", "bonus_type", "num_rvecs", "alpha", "prior_mean", "p"]

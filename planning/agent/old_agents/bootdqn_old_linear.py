...
class BootstrappedDqn():
  ...
  def start(self, observation):
    ...
    self.current_data.flags[self.current_pos,:] = self._rng.binomial(1, self._mask_prob, self._num_ensemble)
    self.time_step += 1
  ...
  def step(self, observation, reward, terminal):
    ...
    if (self.policy_update_frequency != -1 and self.time_step%self.policy_update_frequency == 0) or terminal:
      ...
      self.current_head = self._rng.choice(self._num_ensemble,1)[0]
    next_act = self.select_action(self.features)
    ...
    # Reward noise
    reward_noise = self._rng.randn(self._num_ensemble).astype(np.float32) * self._noise_scale
  ...
  def update_weights(self):
    ...
    indices = self._rng.choice(size,batch_size,replace=False)
...
def make_ensemble(num_actions: int,
  ...
  for _ in range(num_ensemble):
    network = snt.Sequential([
        snt.Linear(num_actions, with_bias=True),
        # snt.Linear(num_actions, with_bias=True, w_init=snt.initializers.TruncatedNormal(mean=0.0, stddev=1.0, seed=None)),
    ])

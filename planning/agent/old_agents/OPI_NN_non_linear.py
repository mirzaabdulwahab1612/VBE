...
class SARSA():
    ...
    def start(self, observation):
        ...
        next_act = self.optimistic_action(copy.deepcopy(self.features), self.wvec, self.uvec, self.rand_pred, target_action=False)
    ...
    def step(self, observation, reward, terminal):
        ...
        next_act = self.optimistic_action(copy.deepcopy(self.features), self.wvec, self.uvec, self.rand_pred, target_action=False, print_flag=True)
        if next_act is None:
            return next_act
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            rewards_ = torch.Tensor(rewards).to(self.device)
    ...
    def optimistic_action(self, state, wvec=None, uvec=None, rand_pred=None, target_action=False, print_flag=False):
        with torch.no_grad():
            ...
            if act is None:
                print(f"target_action: {target_action} val: {val} unc: {unc} iunc: {iunc} unc_bonus: {unc_bonus} iunc_bonus: {iunc_bonus}")

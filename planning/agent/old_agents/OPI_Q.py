...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.policy_rvec = np.zeros((self.num_rvecs, self.mem_size))
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            next_values = get_values(next_states, self.policy_wvec, self.num_action)
            next_q = np.max(next_values, axis=-1)
    ...
    def learn_uncertainty_estimates(self, num_updates=10, mini_batch_size=64):
        ...
        for i in range(num_updates):
            ...
            next_values = get_values(next_states, self.policy_uvec, self.num_action)
            next_q = np.max(next_values, axis=-1)
            ...
            next_values = get_values(next_states.T, (self.rand_wvec - self.policy_rvec), self.num_action, multi=True)
            next_q = np.max(next_values, axis=-1)
    ...
    def optimistic_action(self, state, wvec=None, uvec=None, evec=None, greedy=False, target_action=False, print_flag=False):
        ...
        if self.start_training:
            if(greedy):
                ...
                temp_iunc = []
                iunc = np.abs(get_values(state.T, evec, self.num_action, multi=True))
                if(target_action):

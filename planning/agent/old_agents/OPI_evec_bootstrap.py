...
class SARSA():
    ...
    def __init__(self, params):
        ...
        # self.evec = self.np_random.multivariate_normal(np.zeros(self.mem_size), np.eye(self.mem_size), size=self.num_rvecs)
        ...
        self.policy_evec = np.zeros((self.num_rvecs, self.mem_size))
    ...
    def learn_uncertainty_estimates(self, wvec=None, uvec=None, evec=None, num_updates=10, mini_batch_size=64):
        ...
        for i in range(num_updates):
            ...
            # terminals_ = np.tile(terminals, (self.num_rvecs, 1)).T    
            ...
            # bootstrapping error
            # rnd_target_next_values = get_values(next_states.T, self.rand_wvec, self.num_action, multi=True)
            # rnd_target_next_q = rnd_target_next_values[np.arange(mini_batch_size), :, next_actions]
            rnd_target_current_q = np.dot(states, self.rand_wvec.T)
            # rnd_pred_next_values = get_values(next_states.T, self.rvec, self.num_action, multi=True)
            # rnd_pred_next_q = rnd_pred_next_values[np.arange(mini_batch_size), :, next_actions]
            rnd_pred_current_q = np.dot(states, self.rvec.T)
            rnd_error_current = np.abs(rnd_pred_current_q - rnd_target_current_q)
            # rnd_error_next = np.abs(rnd_pred_next_q - rnd_target_next_q)

            next_values_e = get_values(next_states.T, self.evec, self.num_action, multi=True)
            next_qe = next_values_e[np.arange(mini_batch_size), :, next_actions]
            current_qe = np.dot(states, self.evec.T)
            td_error_rnd = (rnd_error_current + self.gamma*terminals_*next_qe) - current_qe
            batch_errors_rnd = td_error_rnd.T.dot(states)
            self.evec = self.evec + ((self.alpha/mini_batch_size)* batch_errors_rnd)
    ...
    def optimistic_action(self, state, wvec=None, uvec=None, evec=None, greedy=False, target_action=False, print_flag=False):
        ...
        if self.start_training:
            ...
            if(greedy):
                ...
                rand = get_values(state.T, self.rand_wvec, self.num_action, multi=True)
                iunc = iunc*rand
                if(target_action):
                ...
                if(not target_action):                        
                    print(f"val: {val} , iunc: {iunc} , value_new: {value_new}")
        ...
        # if(not target_action):
            # print(f"in sample uncertainty is high for action: {act}")

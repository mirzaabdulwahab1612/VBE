...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.rand_wvec = np.ones(self.mem_size)
    ...
    def learn_uncertainty_estimates(self, wvec=None, uvec=None, evec=None, num_updates=10, mini_batch_size=64):
        ...
        for i in range(num_updates):
            ...
            for state, action, reward, next_state, terminal in zip(states, actions, rewards, next_states, terminals):
                ...
                td_error_w_rand = np.dot(self.yvec, (self.rand_wvec - self.rvec))
            ...
            batch_errors_rand = states.T.dot(td_errors_w_rand)
            ...
            self.rvec = self.rvec + ((1.0/mini_batch_size)* batch_errors_rand)

            # self.evec = np.abs(self.rvec - self.rand_wvec)
            self.evec = self.rand_wvec - self.rvec

            # print(f"self.evec: {self.evec}")

    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            if self.target_polic_type == "optimistic" or self.target_polic_type == "optimistic-uniform":
                next_actions = self.optimistic_action(next_states, self.wvec, target_action=True)
            ...
            target_q = (self.reward_coeff*rewards + self.bonus_coeff*reward_bonus) + self.gamma*terminals_*next_q
    ...
    def optimistic_action(self, state, wvec=None, target_action=False, print_flag=False):
        with torch.no_grad():
            ...
            if self.start_training:
                ...
                #     iunc = torch.abs(rand_p - rand_t)
                #     iunc = torch.max(iunc, axis=0)[0]
                #     print(f"values: {value_new} val: {val} iunc: {iunc}")
            else:

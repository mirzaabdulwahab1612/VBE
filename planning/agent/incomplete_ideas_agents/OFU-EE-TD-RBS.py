...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.intrinsic_wvec = LinearNet(self.feature_dim, self.num_action, zero_init=False)
        self.target_intrinsic_wvec = LinearNet(self.feature_dim, self.num_action, zero_init=False)
        ...
        for i in range(self.num_rvecs):
            # Linear function approximator
            self.rand_target.append(LinearNet(self.feature_dim, self.num_action, prior_mean=self.prior_mean, zero_init=False))
            self.rand_pred.append(LinearNet(self.feature_dim, self.num_action, zero_init=False))
            self.target_rand_pred.append(LinearNet(self.feature_dim, self.num_action, zero_init=False))
        ...
        net_params = list(self.wvec.parameters()) + list(self.intrinsic_wvec.parameters())
        for i in range(self.num_rvecs):
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            rand_p = torch.stack([self.rand_pred[id].forward(states_t).detach() for id in range(self.num_rvecs)])
            rand_t = torch.stack([self.rand_target[id].forward(states_t).detach() for id in range(self.num_rvecs)])
            iunc = torch.abs(rand_p - rand_t)
            iunc = torch.max(iunc, axis=0)[0]
            reward_bonus = iunc[np.arange(mini_batch_size), actions]
            ...
            # should i make the intrinsic value function non-terminating like in ACB, RND, ICML paper?
            target_q = reward_bonus + self.gamma*terminals_*next_q
            ...
            loss = extrinsic_loss + intrinsic_loss
    ...
    def optimistic_action(self, state, wvec=None, intrinsic_wvec=None, target_action=False, print_flag=False):
        with torch.no_grad():
            ...
            if self.start_training:
                ...
                #     rand_p = torch.stack([self.rand_pred[id].forward(hidden_state).detach() for id in range(self.num_rvecs)])
                #     rand_t = torch.stack([self.rand_target[id].forward(hidden_state).detach() for id in range(self.num_rvecs)])
                #     iunc = torch.abs(rand_p - rand_t)
                #     iunc = torch.max(iunc, axis=0)[0]
                #     print(f"values: {value_new} val: {val} intrinsic_val: {intrinsic_val} iunc: {iunc}")
            else:
...
def get_params():
    return ["optimizer_type", "target_polic_type", "rvfs_update_all", "action_type", "num_rvecs", "reward_coeff", "bonus_coeff", "alpha", "prior_mean"]

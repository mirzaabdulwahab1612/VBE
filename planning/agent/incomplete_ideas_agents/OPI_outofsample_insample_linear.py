...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.uvec = LinearNet(self.feature_dim, self.num_action, bias=True)
        self.target_uvec = LinearNet(self.feature_dim, self.num_action, bias=True)
        ...
        for i in range(self.num_rvecs):
            # Linear function approximator
            self.rand_target.append(LinearNet(self.feature_dim, self.num_action, bias=True))
            self.rand_pred.append(LinearNet(self.feature_dim, self.num_action, bias=True))
            self.target_rand_pred.append(LinearNet(self.feature_dim, self.num_action, bias=True))
        ...
        self.pu = params.agent_params.pu
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            if self.target_polic_type == "optimistic" or self.target_polic_type == "optimistic-uniform":
                next_actions = self.optimistic_action(next_states, self.wvec, self.uvec, self.rand_pred, target_action=True)
            ...
            # in-sample uncertainty estimation, the mean and variance should correspond to the environment noise
            reward_uvec = torch.Tensor(self.np_random.normal(loc=0, scale=1, size=mini_batch_size))
            next_uvalues = self.target_uvec.forward(next_states_t).detach()
            next_qu = next_uvalues[np.arange(mini_batch_size), next_actions]
            current_uvalues = self.uvec.forward(states_t)
            current_qu = current_uvalues[np.arange(mini_batch_size), actions]
            target_qu = reward_uvec + self.gamma*terminals_*next_qu
            loss += self.loss_fn(current_qu, target_qu)
    ...
    def optimistic_action(self, state, wvec=None, uvec=None, rand_pred=None, target_action=False):
        with torch.no_grad():
            hidden_state = torch.Tensor(state).to(self.device)
            if self.start_training:
                val = wvec.forward(hidden_state)
                unc = torch.abs(uvec.forward(hidden_state))
                ...
                unc_bonus = (self.pu * unc)
                iunc_bonus = (self.p * iunc)
                uncertainty_bonus = unc_bonus + iunc_bonus
                value_new = (val + uncertainty_bonus).data.cpu().numpy()

                # if(not target_action):
                #     print(f"val: {val} , unc: {unc} , iunc: {iunc} , unc_bonus: {unc_bonus} , iunc_bonus: {iunc_bonus} , uncertainty_bonus: {uncertainty_bonus}")
            else:
...
def get_params():
    return ["optimizer_type", "target_polic_type", "rvfs_update_all", "action_type", "num_rvecs", "alpha", "p", "pu"]

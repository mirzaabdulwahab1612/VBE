...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.wvec = LinearNet(self.feature_dim, self.num_action)
        self.target_wvec = LinearNet(self.feature_dim, self.num_action)
        self.intrinsic_wvec = LinearNet(self.feature_dim, self.num_action)
        self.target_intrinsic_wvec = LinearNet(self.feature_dim, self.num_action)
        # ACB random network and predictors
        self.auxWeights = LinearNet(self.feature_dim, self.numAux, bias=False)
        self.auxWeightsAv = copy.deepcopy(self.auxWeights)
        self.init = self.auxWeights.net.weight.data
    ...
    def learn_one_update(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            ...
            response = self.auxWeightsAv(next_states_t).detach() ** 2
            ...
            rand_predictions = self.auxWeights.forward(samps)
            auxLoss = self.loss_fn(rand_predictions, rand_labels)
            if self.initPull >= 0: auxLoss = auxLoss + self.loss_fn(self.auxWeights.net.weight, self.init) * self.initPull
            self.auxUpdateCount += 1
            
            lamb = 0.
            if self.iterateAve == 1: lamb = (self.auxUpdateCount - 1) / self.auxUpdateCount
            self.auxWeightsAv.net.weight.data = self.auxWeightsAv.net.weight.data * lamb + self.auxWeights.net.weight.data * (1 - lamb)
    ...
    def optimistic_action(self, state, wvec=None, intrinsic_wvec=None, target_action=False, print_flag=False):
        with torch.no_grad():
            ...
            if self.start_training:
                ...
                #     print(f"val: {val} intrinsic_val: {intrinsic_val} value_new: {value_new}")
            else:

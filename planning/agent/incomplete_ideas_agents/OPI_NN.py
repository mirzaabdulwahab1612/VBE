...
class SARSA():
    ...
    def __init__(self, params):
        ...
        self.uvec = Net(self.feature_dim, self.num_hidden_units, self.num_action, zero_init=False)
        self.target_wvec = Net(self.feature_dim, self.num_hidden_units, self.num_action, zero_init=False)
        self.target_uvec = Net(self.feature_dim, self.num_hidden_units, self.num_action, zero_init=False)
    ...
    def learn_out_uncertainty_estimates(self, num_updates=10, mini_batch_size=64):
        for i in range(num_updates):
            # use train buffer to learn out of sample uncertainty
            states, actions, rewards, next_states, terminals = self.replay_buffer_train.sample_batch_lastdata(mini_batch_size)
            next_actions = self.optimistic_action(next_states, self.target_wvec, self.target_uvec, self.target_rand_pred, target_action=True)

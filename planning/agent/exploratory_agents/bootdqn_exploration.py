...
class BootstrappedDqn():
  ...
  def _step(self, transitions: Sequence[tf.Tensor]):
    ...
    o_tm1, a_tm1, _, d_t, o_t, m_t, z_t = transitions
    ...
    with tf.GradientTape() as tape:
      ...
      for k in range(self._num_ensemble):
        ...
        target_y = z_t[:, k] + self._discount * d_t * target_value
...
def make_ensemble(num_actions: int,
  ...
  for _ in range(num_ensemble):
    network = snt.Sequential([
        snt.Linear(num_actions, with_bias=False, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(input_dims, 1/2)), seed=1+rnd_seed+_))
    ])
    prior_network = snt.Sequential([
        snt.Linear(num_actions, with_bias=False, w_init=snt.initializers.TruncatedNormal(mean=prior_mean, stddev=(1.0/np.power(input_dims, 1/2)), seed=2+rnd_seed+_))
    ])
